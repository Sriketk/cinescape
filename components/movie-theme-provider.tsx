...
const MovieThemeContext = createContext<MovieThemeContextType | undefined>(
  undefined
);
  ...
  useEffect(() => {
    ...
    if (theme.effects?.backgroundGlow) {
      root.style.setProperty(
        "--theme-glow-color",
        theme.effects.backgroundGlow.color
      );
      root.style.setProperty(
        "--theme-glow-from",
        theme.effects.backgroundGlow.from
      );
      root.style.setProperty(
        "--theme-glow-to",
        theme.effects.backgroundGlow.to
      );
      root.style.setProperty(
        "--theme-glow-secondary",
        theme.effects.backgroundGlow.secondary
      );
    }

    // Apply font family based on typography settings
    const fontClass =
      theme.typography.primary === "serif"
        ? "font-serif"
        : theme.typography.primary === "mono"
        ? "font-mono"
        : theme.typography.primary === "figtree"
        ? "font-figtree"
        : "font-sans";
